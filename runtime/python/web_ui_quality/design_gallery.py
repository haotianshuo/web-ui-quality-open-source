...
from .design_ui import render_design_ui as render_optional_design_ui
...
def _decision_brief_markdown(report: Mapping[str, Any]) -> str:
    ...
    next_action = (
        ...
        else "当前默认只提供结构化候选和验证证据；需要视觉比较时运行 `web-ui-quality design-ui <upgrade-output>`，再下载决策文件交给 Codex。"
    )
    lines = [
        "# 设计决策简报",
        "",
...
def render_design_gallery(report: Mapping[str, Any], gallery_dir: str | Path) -> dict[str, str]:
    """Render the optional decision workbench for an existing design report."""
    return render_optional_design_ui(
        report,
        gallery_dir,
        html_renderer=_gallery_html_v372,
        brief_renderer=_decision_brief_markdown,
    )
...
def build_design_gallery(
    ...
    *,
    ...
    if render_ui:
        ...
        rendered["ui"] = {"requested": True, "status": "RENDERED"}
        ui_info = {"requested": True, **render_design_gallery(rendered, gallery_root)}
    report["ui"] = ui_info
