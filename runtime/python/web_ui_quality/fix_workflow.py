...
<section class=\"card\"><h2>计划解决</h2><ul>{goals}</ul></section><section class=\"card\"><h2>源码范围</h2><ul>{files}</ul></section>
...
def prepare_fix_workflow(
    ...
    *,
    ...
    scope = explicit or mapped
    source = "explicit-host-scope" if explicit else "finding-evidence" if mapped else "unconfirmed"
    ...
    status = "CHANGE_BUDGET_EXCEEDED" if budget_blocked else "PATCH_CANDIDATE_REQUIRED" if confirmed else "SCOPE_NOT_CONFIRMED"
    plan: dict[str, Any] = {
        "schemaVersion": "2",
        ...
        "goal": goal,
        "userTitle": "修改范围超过预算" if budget_blocked else "已确认修复源码范围" if confirmed else "尚不能安全确定修改文件",
        ...
        "hostBridgeAction": "submitPatchCandidate" if confirmed and not budget_blocked else "submitFixApproval",
        "sourceProjectChanged": False,
        "nextAction": (
            "当前候选范围超过 Change Budget；必须由用户/Host 明确扩大范围后生成新的绑定计划，不能沿用当前授权。"
            if budget_blocked else
            "由 Host 生成并展示精确 Patch Candidate，完成写入后回传 Hash Receipt。" if confirmed else
            "请先把 Finding 映射到 Route、Selector、Component、Source File 和 Source Range；不得按文件名猜测。"
        ),
